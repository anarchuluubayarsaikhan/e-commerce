...
export function Login() {
  ...
  function onSubmit() {
    postFetch("login", {email: formik.values.email, password: formik.values.password}).then((res)=> res.json()).then(({token}) => localStorage.setItem("accessToken", token));
    formik.resetForm()
  }
  ...
  return (
   <div className="flex flex-col gap-4 m-auto">
    ...
    <div className="flex flex-col gap-12">
       <Link href="/forgotpass" className="self-center underline text-#71717A text-sm font-normal">Нууц үг мартсан</Link>
       <Button className="bg-white border-primaryBlue border rounded-[18px] text-primaryBlue text-sm font-medium hover:bg-gray-50 max-w-[334px]">
         Бүртгүүлэх
       </Button>
     </div>
   </div>
